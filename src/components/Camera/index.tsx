...
export default class Camera extends PureComponent<Props, State> {
  ...
  public stop() {
    if (this.currentMediaStream) {
      ...
      this.currentMediaStream.getVideoTracks().map(track => track.stop());
    } else if (this.currentMediaStream) {
      ((this.currentMediaStream as unknown) as MediaStreamTrack).stop();
    }
    if (
      ...
      this.videoElement.current.srcObject.getVideoTracks().map(track => track.stop());
    } else if (this.videoElement.current && this.videoElement.current.srcObject) {
      ((this.videoElement.current.srcObject as unknown) as MediaStreamTrack).stop();
    }
  }
  ...
  render() {
    ...
    return (
      <div className={classNames('lidig-cameraBody', className)}>
        <video
          autoPlay
          playsInline
          className={'lidig-cameraVideo'}
